...
class BioreactorDataset:
    ...
    def _build_product_mapping(self) -> None:
        """Build mapping from product names to indices."""
        unique_products = sorted(self.df[self.PROD_ID_COL].unique())
        self.prod_map = {name: idx for idx, name in enumerate(unique_products)}
    ...
    def _split_data(self) -> None:
        ...
        # Create validation split (10% of training pool)
...
class EarlyStopping:
    ...
    def __call__(
        ...
        if self.best_score is None or score > self.best_score + self.delta:
            ...
            self._save_checkpoint(model, stats)
            self.counter = 0
        else:
            self.counter += 1
    ...
    def _save_checkpoint(self, model: nn.Module, stats: Optional[Dict]) -> None:
